...
class SupervoxelDendrogram {
 private:
  ...
  auto emplace_back(const segid_t &segid0, const segid_t &segid1,
                    const aff_edge_t &weight) {
    ...
    for (size_t z = 0; z < fragments.shape(0); z++) {
      for (size_t y = 0; y < fragments.shape(1); y++) {
        for (size_t x = 0; x < fragments.shape(2); x++) {
          ...
          if (z > 0 && fragments(z - 1, y, x) > 0 &&
              segid != fragments(z - 1, y, x)) {
            auto key = minmax(segid, fragments(z - 1, y, x));
            rg[key].first += 1;
            rg[key].second += affs(2, z, y, x);
          }

          if (y > 0 && fragments(z, y - 1, x) > 0 &&
              segid != fragments(z, y - 1, x)) {
            auto key = minmax(segid, fragments(z, y - 1, x));
            rg[key].first += 1;
            rg[key].second += affs(1, z, y, x);
          }

          if (x > 0 && fragments(z, y, x - 1) > 0 &&
              segid != fragments(z, y, x - 1)) {
  ...
  auto greedy_mean_affinity_agglomeration(const AffinityMap &affs,
    ...
    while (!heap.empty()) {
      ...
      if (meanWeight < minThreshold) {
        std::cout << "reached mean affinity lower than threshold: "
                  << meanWeight << std::endl;
        break;
      }
 ...
 public:
  SupervoxelDendrogram(const PyAffinityMap &affs,
                       const PySegmentation &fragments_,
                       const aff_edge_t &minThreshold)
