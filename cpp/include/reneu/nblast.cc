...
class ScoreTable {
 private:
  ...
  // const DistThresholdsType distThresholds = {0., 0.75, 1.5, 2, 2.5, 3, 3.5,
  // 4, 5, 6, 7, 8, 9, 10,
 ...
 public:
  ...
  inline auto operator()(const float &dist, const float &adp) const {
    ...
    // minus a small value to make sure that adp=1, we get index 9 rather than
    // 10
    ...
    // if (adpIdx != adpIdx2){
    // std::cout<< adp << ": inconsistent adpidx: " << adpIdx << ", " << adpIdx1
    // << ", " << adpIdx2 << std::endl;
    // }
  ...
  auto self_score() const { return table(0, 9); }
...
class VectorCloud {
 private:
  auto construct_vectors(const Index &nearestPointNum) {
    ...
    // find the nearest k points and compute the first principle component as
    // the main direction
    ...
    for (Index pointIdx = 0; pointIdx < pointNum; pointIdx++) {
      xt::xtensor<float, 1> queryPoint =
          xt::view(points, pointIdx, xt::range(0, 3));
      auto nearestPointIndices = kdTree.knn(queryPoint, nearestPointNum);
      // std::cout<< "nearest point indices: " << nearestPointIndices <<
      // std::endl;
...
public:
  auto query_by(const VectorCloud &query, const ScoreTable &scoreTable) const {
    ...
    xt::xtensor_fixed<float, xt::xshape<3>> queryPoint, nearestPoint,
        queryVector, targetVector;

    const auto queryPoints = query.get_points();
    const auto queryVectors = query.get_vectors();
    for (Index queryPointIndex = 0; queryPointIndex < query.size();
         queryPointIndex++) {
      ...
      // absoluteDotProduct = std::abs(xt::linalg::dot( queryVector,
      // targetVector )(0));

      // lookup the score table and accumulate the score
      rawScore += scoreTable(distance, absoluteDotProduct);

      // std::cout<< "\n "<< queryPointIndex << " th query point: " <<
      // queryPoint <<std::endl; std::cout<< "nearest point index: "<<
      // nearestPointIndex << " : " << nearestPoint << std::endl; std::cout<<
      // "vectors: "<<queryVector << "   "<< targetVector << std::endl;
      // std::cout<< "distance: "<< distance << ";   absolute dot product: "
      //                                     << absoluteDotProduct << std::endl;
      // std::cout<< "accumulate score: "<< scoreTable(distance,
      // absoluteDotProduct) << " to " << rawScore<< std::endl;
    }
    return rawScore;
  }
};  // VectorCloud class
