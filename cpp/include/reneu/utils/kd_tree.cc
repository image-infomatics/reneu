...
class IndexHeap {
 private:
  ...
  std::priority_queue<HeapElement, std::vector<HeapElement>, decltype(cmp)>
      pqueue;
 ...
 public:
  ...
  inline auto size() const { return pqueue.size(); }

  inline auto max_squared_dist() const { return pqueue.top().first; }
 ...
 * this design will avoid class inheritance and virtual functions, smart
 * pointers... this is more efficient according to libnabo paper: Elseberg, Jan,
 * et al. "Comparison of nearest-neighbor-search strategies and implementations
 * for efficient shape registration." Journal of Software Engineering for
 * Robotics 3.1 (2012): 2-12.
 */
class KDTreeNode {
 private:
 ...
 public:
  ...
  inline auto get_dim() const { return dim_child_bucketSize >> DIM_BIT_START; }
  ...
  void print() const {
    if (is_leaf()) {
      ...
      std::cout << "right child node index: " << get_right_child_node_index()
                << std::endl;
...
class KDTree {
 private:
  ...
  Index build_kd_nodes(const Points &points, const PointIndices &pointIndices) {
    ...
    if (pointIndices.size() <= leafSize) {
      ...
      // std::cout<< "\ninserting leaf node to " << kdTreeNodes.size() <<
      // std::endl; node.print(); std::cout<<"\nexisting nodes in kdTreeNodes:
      // "<<std::endl; for(auto n : kdTreeNodes){ n.print();
      // }
      ...
      xt::xtensor<float, 1> coords =
          xt::index_view(xt::view(points, xt::all(), dim), pointIndices);
      ...
      const PointIndices sortedPointIndices =
          xt::index_view(pointIndices, argSortIndices);
      ...
      const auto leftPointIndices =
          xt::view(sortedPointIndices, xt::range(0, splitIndex));
      const auto leftNodeIndex = build_kd_nodes(points, leftPointIndices);
      assert(leftNodeIndex == nodeIndex + 1);

      const auto rightPointIndices =
          xt::view(sortedPointIndices, xt::range(splitIndex, _));
  ...
  void knn_update_heap(const Point &queryPoint, IndexHeap &indexHeap,
    ...
    if (indexHeap.max_squared_dist() <
        bbox.min_squared_distance_from(queryPoint)) {
      return;
    }
    ...
    if (node.is_leaf()) {
      ...
      auto leafPoints =
          xt::view(pointsBucket, xt::range(bucketStart, bucketStop), xt::all());
      // Points minusPoints = leafPoints - queryPoint;
      // xt::xtensor<float, 1> squaredDistances = xt::norm_sq( leafPoints -
      // queryPoint, 1.0, xt::evaluation_strategy::immediate)();

      // std::cout<< "leafPoints - queryPoint: " << (leafPoints - queryPoint) <<
      // std::endl; std::cout<< "squared distances: " << squaredDistances <<
      // std::endl; std::cout<< "bucket start: " << bucketStart << ", bucket
      // stop: "<< bucketStop <<
 ...
 public:
  KDTree(const KDTreeNodes &kdTreeNodes_,
         const PointIndicesBucket &pointIndicesBucket_,
         const PyPoints &pointsBucket_, const Index &leafSize_)
      : kdTreeNodes(kdTreeNodes_),
        pointIndicesBucket(pointIndicesBucket_),
        pointsBucket(Points(pointsBucket_)),
        leafSize(leafSize_) {}

  KDTree(const std::tuple<KDTreeNodes, PointIndicesBucket, PyPoint, Index> &tp)
      : kdTreeNodes(std::get<0>(tp)),
        pointIndicesBucket(std::get<1>(tp)),
        pointsBucket(Points(std::get<2>(tp))),
        leafSize(std::get<3>(tp)) {}
  ...
  auto get_kd_tree_nodes() const { return kdTreeNodes; }

  auto get_point_indices_bucket() const { return pointIndicesBucket; }

  auto get_py_points_bucket() const { return PyPoints(pointsBucket); }

  auto get_leaf_size() const { return leafSize; }
