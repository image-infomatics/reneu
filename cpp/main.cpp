...
PYBIND11_MODULE(libreneu, m) {
  ...
  py::class_<Skeleton>(m, "XSkeleton")
      ...
      .def_property("points", &Skeleton::get_py_points,
                    &Skeleton::set_py_points)
      ...
      .def("translate_centroid_to_origin",
           &Skeleton::translate_centroid_to_origin)
      ...
      .def(py::pickle(
          ...
          [](py::tuple tp) {  // __setstate__
            if (tp.size() != 2) throw std::runtime_error("Invalid state!");
  ...
  py::class_<ScoreTable>(m, "XNBLASTScoreTable")
      ...
      .def("__getitem__",
           py::overload_cast<const std::tuple<float, float> &>(
               &ScoreTable::operator()),
           "get table item");
  ...
  py::class_<VectorCloud>(m, "XVectorCloud")
      ...
      .def(py::pickle(
          [](const VectorCloud &vc) {  // __getstate__
            ...
            return py::make_tuple(vc.get_py_points(), vc.get_py_vectors(),
                                  vc.get_kd_tree_serializable_tuple());
          },
          [](py::tuple tp) {  // __setstate__
            if (tp.size() != 3) throw std::runtime_error("Invalid state!");
  ...
  py::class_<NBLASTScoreMatrix>(m, "XNBLASTScoreMatrix")
      ...
      // Note that the conversion from python list to std::vector has copy
      // overhead
      .def(py::init<const std::vector<VectorCloud> &, const ScoreTable &>())
      .def_property_readonly("raw_score_matrix",
                             &NBLASTScoreMatrix::get_raw_score_matrix)
